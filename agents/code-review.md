...
**Ticket mode** (`ticket-lifecycle`): per-stage focused lint/unit/contract/schema checks with RED-to-GREEN replay, assertion delta, scope drift, and commit-protocol validation. The test-only RED commit must precede the production-only GREEN commit; mixed commit scopes, missing SHAs, or a dirty worktree are blockers. The final `all_stages: true` gate runs the **full test suite** via the compose backend before `state:ready-for-ticket-review`; the local CodeRabbit pre-flight (`execution_mode: ticket_coderabbit_preflight`) runs once before the sub-PR opens — correctness/obvious-bugs/risky-changes scope, findings returned as fix-now suggestions.
...
When `sandbox_id` is provided in the Task contract, reuse the sandbox via the **plugin tool `sandbox_run_test`** from `plugins/sandbox.js` (with the same `sandbox_id` + `compose_test_file` from the coder session's §0.3). Do not write `docker compose` invocations yourself — the plugin owns the invocation form. Destroy the sandbox after `APPROVED` verdict or on `ENV_BLOCKED` via the plugin tool **`sandbox_destroy`**. Keep the sandbox alive on `BLOCKED` — the developer will retry and needs the same sandbox.

Return `report_to_parent` with `verdict: APPROVED|NEEDS_CHANGES|BLOCKED`, criterion coverage, tests, scope findings, security status, residual risks, and `commit_protocol` evidence for the RED/GREEN pair and any amendments (plus the CodeRabbit report shape and `completion_summary` where they apply — see `skills/code-review/SKILL.md`). Do not edit files, never invoke `autofix`, never commit.
