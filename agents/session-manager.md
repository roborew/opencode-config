...
3. **`admitted` keys on HTTP 204.** A successful injection returns `{ok: true, admitted: true, status: 204, session_id}`. Anything else is a failure — surface `manualRecovery` from the envelope verbatim to the parent. **Both `agent_match` and `directory_match` must be `true`** for the kickoff to be `admitted`; a mismatch on either is a hard stop (not advisory) and the orchestrator pauses the batch per `BLOCKED: KICKOFF_DIRECTORY_BIND_FAILED` / `BLOCKED: KICKOFF_AGENT_BIND_MISMATCH`. **The bind evidence is a `session_list({})` re-list after a 750 ms settle delay (not the create envelope alone)** — the server's create response has the same eventual-consistency window as the rest of the API.
...
5. **No matching session** — handle per `create_if_absent`:
   - **`create_if_absent === true` (default):** call `session_create({ directory, agent, title: "ticket coder session" })`. The plugin forwards `directory` as `?directory=...` on the POST URL; the server binds the new session to that worktree directory. Use the envelope's `session_id` as the **provisional** chosen id, then re-list to confirm. The create envelope is secondary evidence; the post-settle re-list is the canonical signal.
      1. If `r.ok === false` → surface `SESSION_API_FAILED` (verbatim from the envelope's `error` / `manualRecovery`).
      2. **Settle delay**: `await sleep(750)` — empirically wide enough to clear the eventual-consistency window on the 1.18.25-1.18.26 server family per the operator's report.
      3. Call `session_list({})` (no args — server returns everything, client filters). Find the entry by id.
         - **Missing after settle** (3 attempts, 500 ms backoff) → surface `{ok: false, admitted: false, action: "kickoff", error: "create_response_missing_id", blocker_code: "KICKOFF_DIRECTORY_BIND_FAILED", session_id: r.session_id, target_directory: r.target_directory, requested_directory: directory, manualRecovery: <curl snippet for POST /session with ?directory=...>}`.
         - **Returned `directory` !== args.directory** → surface `{ok: false, admitted: false, action: "kickoff", error: "directory_bind_failed", blocker_code: "KICKOFF_DIRECTORY_BIND_FAILED", session_id, target_directory: list_row.directory, requested_directory: directory, manualRecovery: <curl snippet for POST /session with ?directory=...>}`. The session exists but is not bound to the worktree dir — coder would load the wrong repo context. This is the same `blocker_code` the orchestrator's Global Invariant #8 watches.
         - **Returned `agent` !== args.agent** → surface `KICKOFF_AGENT_BIND_MISMATCH` analogously.
         - **All match** → set `resolution: "created"`, `reused: false`, `agent_match: true`, `directory_match: true`, `bind_failed: false`, `session_id: list_row.id`, `target_directory: list_row.directory`. Continue to step 6 (inject).
