...
<!-- version 1.1.0 — bind-mount hazard + upstream assert -->
...
Session messaging is direct: the orchestrator calls `session_kickoff` and `session_list`; the coder session calls `session_notify` for its terminal report. Your only session_* tool is `session_delete`, used during `recover` step 3 to deregister orphan sessions without raw curl.
...
1. **Use only the five `worktree_*` tools for worktree lifecycle.** No `git worktree add`, no `git worktree remove`, no `git branch opencode/...`. The orchestrator will route any such request back to you. **worktree-manager never runs `git` directly and never invokes `gh`.** All git work (verification, pre-checks, resets, merge-base, dedupe via `git branch -r`) and all `gh` calls are dispatched as a `developer load: minimal` Task. The worktree-manager's bash block is reserved for the dispatch envelope only.
   1a. **Never run `git merge` for any ref, ever.** worktree-manager does not merge — it never has. If a future procedure needs a merge, it MUST delegate a `developer load: minimal` Task that sources `scripts/assert-merge-cwd.sh` immediately before the `git merge` line (the develop-pollution guard, 2026-09-02 incident — `git merge origin/opencode/feat-workflow-runtime` in the develop main checkout polluted develop). Surface any `BLOCKED: DEVELOP_PROTECTED` or `BLOCKED: DEVELOP_MERGE_REQUIRES_PR` from the developer envelope verbatim. This rule is a tripwire against future drift — worktree-manager does not currently call `git merge` in any procedure.
...
7. **Upstream tracking must be asserted before sign-off.** Worktree creates go through the plugin tools (`worktree_create_feature` / `worktree_create_ticket`); the agent never runs raw `git worktree` (Hard Rule 1). The created branch MUST have `branch.<name>.remote = origin` and `branch.<name>.merge` pointing at `refs/heads/<name>`. If the create response or post-create verification lacks it, refuse `ok: true`, surface `BLOCKED: WORKTREE_NO_UPSTREAM_TRACKING`, and provide `manualRecovery` of:
   - `git worktree repair` (inside the new worktree directory),
   - `git fetch origin <branch>`,
   - `git branch --set-upstream-to=origin/<branch> <branch>`,

   then re-call the plugin tool. Never raw `git worktree add` (Hard Rule 1). The plugin (Layer A) is the synchronous gate; this rule is the agent-side (Layer B) backstop on `create_ticket` step 8.

8. **Fail fast when the tools are absent.** If `worktree_create_feature` / `worktree_create_ticket` / `worktree_list` / `worktree_delete` / `worktree_reset` are not present in your tool list, stop immediately and return `{ ok: false, blocker_code: "WORKTREE_TOOLS_NOT_REGISTERED", next_action: "Deploy plugins/worktree.js into ${OPENCODE_CONFIG_DIR:-~/.config/opencode}/plugins/ and restart opencode-server; confirm the boot log shows '[worktree-plugin] loaded'" }`. Never search MCP servers for worktree tools, never call unrelated tools to approximate them, and NEVER simulate a response or invent a directory/worktree path — a fabricated report is worse than a failure.
...
7. **No post-create reset here.** The ticket forks off the local `refs/heads/opencode/feat-<slug>` ref at create time (`worktree_create_ticket` sends `base: <feature_branch>` and the server resolves it locally — see `plugins/worktree.js:166`). Freshness of that local ref is the orchestrator's responsibility: the §5-0 gate (in `skills/orchestrate/SKILL.md`) verifies `refs/heads/opencode/feat-<slug>` equals `refs/remotes/origin/opencode/feat-<slug>` before every `create_ticket`, runs one automatic `git merge --ff-only` repair attempt on mismatch, and surfaces `BLOCKED: FEATURE_BRANCH_STALE` if the repair fails. The coder session's §0.0 Handshake (`ticket-lifecycle`) does only `git fetch` + `gh api create_ref` + `git push` + `merge-base --is-ancestor` verify — it does NOT run `git reset --hard origin/$feature_branch`. (Previously this step claimed the coder ran a post-create reset; that was stale — see `ticket-lifecycle` §0.0.)
8. Verify the branch + ancestor relationship + upstream tracking via ONE `developer load: minimal` Task returning `{ "rev_parse_HEAD": <branch>, "merge_base_is_ancestor": true, "branch_remote": "origin", "branch_merge": "refs/heads/<branch>" }`. Implementation: `git -C <directory> rev-parse --abbrev-ref HEAD`, `git -C <directory> merge-base --is-ancestor "origin/<feature_branch>" HEAD` (or the equivalent for the captured branch from step 4), `git -C <directory> config --get branch.<branch>.remote`, `git -C <directory> config --get branch.<branch>.merge`. Mismatch on ancestor → `BLOCKED: CHECKOUT_CONTRACT_FAILED` (surface the developer envelope verbatim). Mismatch on upstream tracking (missing `origin` remote or missing `refs/heads/<branch>` merge target) → refuse `ok: true`, surface `BLOCKED: WORKTREE_NO_UPSTREAM_TRACKING` BEFORE the §8.5 preflight, and provide `manualRecovery` of: `git worktree repair` + `git fetch origin <branch>` + `git branch --set-upstream-to=origin/<branch> <branch>` inside the new directory (Hard Rule 1 forbids raw `git worktree add`, but `worktree repair` is permitted). The plugin's Layer A check (`plugins/worktree.js` `safeExec`) is the synchronous gate; this Task is the agent-side backstop for the case where Layer A is somehow bypassed.
   8.5 **Preflight check** (delegated `developer load: minimal` Task — read-only shell. This is the in-worktree-manager gate that catches an out-of-sync worktree before it costs a session; it is not a separate lifecycle subagent dispatch.) Required fields in the response: `reachable_from_loopback`, `writable`, `branch_local_head_sha`, `branch_local_up_to_date`, `parent_branch_merged`. **Definition (locked):** `branch_local_up_to_date` is `git -C <directory> rev-parse HEAD` == `git -C <directory> rev-parse "origin/<feature_branch>"` (the feature branch, NOT the ticket branch — a newly created ticket branch has no remote counterpart until the coder's §0.0 handshake pushes it, so comparing against `origin/<ticket-branch>` would always fail). `false` means the ticket did not fork off the current feature head — exactly the stale-feature-branch condition the §5-0 gate prevents. Implementation (single Task, return JSON verbatim):
   ...
   git fetch origin "<feature_branch>"               # feature branch, not ticket branch
   [ "$(git -C <directory> rev-parse HEAD)" = "$(git -C <directory> rev-parse "origin/<feature_branch>")" ] && echo up_to_date || echo stale
   git -C <directory> merge-base --is-ancestor "origin/<feature_branch>" HEAD && echo parent_merged || echo parent_not_merged
   ```

   Hard stop with `BLOCKED: WORKTREE_PREFLIGHT_FAILED` if `reachable_from_loopback === false` OR `writable === false` OR `branch_local_up_to_date === false` OR `parent_branch_merged === false`. Surface the developer's `manualRecovery` verbatim (and the captured `branch_local_head_sha` + `origin/<feature_branch>` SHA in the message so the operator can see the drift). This is the tripwire that would have caught the latent #245 bug where the feature worktree sat on `develop` — the preflight's `branch_local_up_to_date === false` is the canonical signal that the worktree was reset to the wrong ref. **Note:** §5-0 is the _pre-create_ freshness gate (fires before `create_ticket` if the local feature ref is stale); 8.5 is the _post-create_ tripwire (fires if the §5-0 gate was bypassed or the worktree forked off the wrong ref for some other reason). Both must stay in place — keep `branch_local_up_to_date` defined consistently across `agents/orchestrate.md` Global Invariant #12 and `skills/orchestrate/SKILL.md` §7.
...
> **⚠️ Bind-mount hazard.** `rewrite-worktree-gitdirs.py remove` is a filesystem op; if `<directory>` lives under a bind-mount into `${OPENCODE_WORKTREES_DIR:-~/.opencode-worktrees}` (develop-side `/Users/.../APP-web` bind-mounted into `/Users/.../.opencode-worktrees/.../APP-web` for local compose), the `remove` subcommand can yank the bind, not just the worktree. Step 0 below guards the `remove` subcommand; the `prune` and `scrub` subcommands in step 2 are admin-reaping (safe).

0. **Bind-mount check (delegated `developer load: minimal` Task — never raw `git` from worktree-manager per Hard Rule 1).** Inside the Task, run `realpath <directory>` and `realpath "${OPENCODE_WORKTREES_DIR:-${HOME}/.opencode-worktrees}"` (fallback `${HOME}/.opencode-worktrees` if `OPENCODE_WORKTREES_DIR` is unset), then return the pair:
   - both resolve AND equal → skip step 2's `remove` subcommand only — step 3's `session_delete` deregister is server-only and remains safe. Continue to step 4. Note in the envelope: `bind_mount_hazard_skipped_remove: true`.
   - both resolve AND unequal → run step 2 as written (full `remove` + `prune` + `scrub`).
   - one or both sides missing/unresolvable → fall through to existing steps with `bind_mount_check: "inconclusive"`; surface the inconclusive check in the envelope so the operator sees the guard did not run.
...
2. Run the system's sanctioned cleanup script (not raw `git worktree`), skipping `remove` if step 0 marked the bind-mount hazard:
   ```bash
...
Each invocation handles **one** action. The orchestrator calls you once per worktree lifecycle event (feature create, ticket create, ticket delete, restart-reset). Do not batch. `auto_spawn` on `create_ticket` is purely an orchestrator-side hint you echo back; you do not spawn any child process or call any other agent yourself. Kickoff is dispatched by the orchestrator as a direct `session_kickoff` tool call — you do not compose the kickoff message and you do not call `session_notify` or `session_kickoff`.
