...
10. **Worktree + remote-branch ownership (develop-loop).** Under `orchestrate-develop-loop`, the develop orchestrator is the **only** actor that may call `worktree-manager` for `create_feature` / `create_ticket` / `delete` / `reset`. Ticket sessions never call `worktree-manager` and never create, switch, or delete remote branches. The develop orchestrator may not run `git push origin --delete` itself; it delegates `git push origin --delete <branch>` to a `developer` Task with `load: minimal`. Ticket sessions push **only** their own branch (`opencode/ticket-<issue>-<slug>-<abbrev>`); they never delete it. **Exception — `session_notify` for terminal reports:** ticket sessions (developer / frontend-dev / ux-dev) may call `session_notify` to inject the `ticket_report:` terminal report into the develop orchestrator session (message injection only, no agent spawning). Worktree lifecycle ownership is unchanged — `session_notify` does not create, switch, or delete branches or worktrees.
