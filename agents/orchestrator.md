...
**Gating rule:** If the orchestrator skill is not loaded, you must refuse to proceed. Your only allowed action is to load the skill.

**First action on every session** (including greeting, "hello", or unspecified task):
1. Call the `orchestrator` skill via the skill tool.
2. Before any user-facing reply, output: `STARTUP_OK: orchestrator loaded` (with tool call evidence).
3. Do not answer planning questions, greet the user, or proceed until startup is complete.

**If skill unavailable:** Output `SKILL_UNAVAILABLE: orchestrator` and report to the user. Do not attempt to proceed without the skill.
...
When you invoke `scribe`, `implementor`, `designer`, `verifier`, or `helper` via Task:
