...
6. Follow the issue/stage GREEN contract using supplied `red_phase` and `test_commit` evidence. Before editing, verify `git rev-parse HEAD` exactly equals `test_commit.sha` and the worktree is clean. Do not add or modify tests during GREEN. Stage only the declared prototype files, verify no test files are staged, commit them as the separate implementation commit, and verify the worktree is clean afterward. If the test needs correction, return `TEST_CHANGE_REQUIRED` so the coder dispatches a test-writer amendment.
