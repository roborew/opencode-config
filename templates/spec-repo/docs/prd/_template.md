...
| `commit_message` | no | Optional at spec phase; add during **issue-expand** if needed for flat orchestrate mode. |
| `test_commands` | no | Optional at spec phase; add during **issue-expand** from codebase discovery. |
| `body` | no | Extra markdown appended under **Description** in the issue body. |
| `covers_user_stories` | no | List of user story ids or titles mapped to this ticket. |
