...
modelTier: "fast"
roleReminder: "Loaded by the `orchestrate` primary agent on the develop branch. The orchestrator never executes tickets — coder sessions do. Wake contract: in-session `session_notify` (primary), `DEV_LOOP_WAKE` from the ticket-only poller fallback, any user message → run `dev-loop-watch.sh` first."
...
while true:
  ...
  #                       (ii) ticket-only poller scripts/dev-loop-poller.sh firing DEV_LOOP_WAKE,
...
The message is a short pointer — truncation-proof by design. The coder session reconstructs the full payload from the kickoff message itself + the branch + GitHub.
...
worktree: <abs path>
repo: OWNER/REPO
develop_session_id: <ctx.sessionID>          # present and non-null
orchestrator_session_id: <ctx.sessionID>     # duplicate for clarity
orchestrator_agent: orchestrate
orchestrator_directory: <main checkout abs path>
return_channel: session_notify               # the only outbound tool you have for me
return_target: { sessionID: <ctx.sessionID>, agent: orchestrate }
durable_channel: ticket_report: comment on OWNER/REPO#<n>  # used if return_channel is unreachable

Load skill ticket-lifecycle and begin. The GitHub issue body (opencode-task-yaml) is the source of truth for stages[], acceptance, and test commands. Reconstruct anything missing per ticket-lifecycle §0 Bootstrap.
```

Compose it once per ticket; pass it verbatim to `session_kickoff` as `message`. The plugin does scoped-list-then-reuse-or-create-then-inject atomically (never falls back to the unfiltered global session list — that path caused the self-resolve bug). The `kickoff` returns `session_source: reused|created` and `resolution: reused|created` so the orchestrator can audit which path was taken; the orchestrator's only success gate remains `admitted: true`. The kickoff message is the contract.

### §5a.0 Handshake contract (mandatory, opt-in via message shape)

The first message a coder receives from the orchestrator is a kickoff
pointer. The pointer is one-way by default (orchestrator -> coder). For
bidirectional chatter (status pings, "do you want me to continue?",
mid-stage blockers) the kickoff MUST carry a handshake block:

return_channel: session_notify
return_target: { sessionID: <ctx.sessionID>, agent: orchestrate }

Without the return_target block, the coder has no way to message the
orchestrator. The coder's only outbound channel is session_notify, and
session_notify in sessionID-mode without a sessionID is a client error
("requires exactly one of sessionID or directory"). Coder sessions in
the wild have stalled on this exact error at least once (see ticket
259, 2026-09-07) because the kickoff pointed at a stale
develop_session_id and the coder's reply went nowhere.

Rules:

- develop_session_id MUST be non-null at kickoff time. If
  <ctx.sessionID> is unavailable (e.g. resuming into a pre-existing
  worktree whose original orchestrator session is gone), the
  orchestrator MUST create a fresh handshake session for return traffic
  (see §5a.1) before kicking the coder, OR fall back to
  durable_channel (issue comment) only and instruct the coder not to
  use session_notify in sessionID-mode.
- The coder's first reply in a new kickoff MUST be a handshake ack
  ("ack, return channel understood, durable channel understood, will
  ping you at <ctx.sessionID> on mid-stage blockers and post a
  ticket_report: comment on terminal") via session_notify to
  <ctx.sessionID>. No work begins until this ack is admitted.
- The orchestrator's lifecycle log entry for the ticket MUST record
  handshake_acked: true on receipt of the ack. session_kickoff never
  waits for the ack (it admits on /prompt_async 204, not on a reply),
  so the ack is async. The first inbound session_notify from the new
  coder session before any ticket_report: comment counts as the ack.

### §5a.1 Resume kickoff (worktree exists, prior coder dead or stale)

If §5a-i (create_ticket) returns an existing worktree OR the
ticket-lifecycle §0.0 handshake step 6 reports a pre-existing
worktree with no live coder session, the orchestrator MUST:

1. NOT re-kickoff with develop_session_id set to a stale value.
   Stale IDs in the kickoff message produce one-way channels and
   silent stalls.
2. Compute a fresh develop_session_id (= ctx.sessionID of THIS
   orchestrator turn) and construct the kickoff message in §5a shape
   with the fresh ID, the return_target block, and an explicit
   "HANDOFF FROM PRIOR CODER" preamble carrying the handoff context
   (status, dirty-tree commit SHA, environmental notes, blockers).
3. Commit any dirty working tree as a single WIP commit on the
   ticket branch and push to origin BEFORE the kickoff, so the fresh
   coder lands on a clean tree. The orchestrator (delegated developer)
   owns this commit, not the coder — implementer children only git
   add. See ticket 259, 2026-09-07, commit deaeba9, for the
   canonical example.
4. Run the KICKOFF_ALREADY_DELIVERED precheck (§5a-iii) — it MUST
   pass (no prior ticket_report: on the issue) before re-kicking.
   On the ticket 259 resume the issue had 0 comments so the check
   passed. If it fails, surface verbatim and pause.
5. Use session_kickoff in default mode (create_if_absent: true). The
   fresh coder will be a NEW session; the prior session is sent a
   standdown message via session_notify in sessionID-mode once the
   fresh kickoff is admitted.
...
- After kicking the batch, **end the turn**. Wakes (in priority order):
  ...
  2. **Poller `DEV_LOOP_WAKE`** — `scripts/dev-loop-poller.sh` (server-host cron) is the **ticket-only durable fallback**; it detects `ticket_report:`-driven ticket deltas and wakes the develop orchestrator.
...
### §5b.1 Inbound coder visibility (mandatory inbound dispatch)

Wake envelopes now carry the new `coder_progress:` and `coder_needs_help:` markers (ticket mode) and `feature_progress:` / `feature_needs_help:` (feature mode). On inbound message:

- Starts with `coder_progress:` → log to lifecycle log as a stage-boundary tick. No action required unless `next_action_required != none` — then surface the ask to the operator verbatim. The matching durable `coder_progress:` comment is also parsed from the issue by `scripts/dev-loop-watch.sh` / `scripts/dev-loop-poller.sh` and surfaces as `coder_progress: { <issue>: <latest event> }` in the wake envelope delta.
- Starts with `coder_needs_help:` → dispatch ONE `developer` Task (`load: minimal`) to run `scripts/dev-loop-watch.sh <slug> --repo <repo>` (idempotent — it surfaces the matching durable comment if any), then execute the embedded `resume_instruction` verbatim. If the instruction is "spawn new session" (the recovery path for "ran out of steps" — the previous coder session is gone), dispatch `session_kickoff` for the worktree with a fresh `load: ticket-lifecycle` (or `feature-review`) pointer; the new coder reconstructs from the durable progress comments + branch.
- Starts with `DEV_LOOP_WAKE:` → existing handler.
- Starts with `ticket_report:` / `feature_report:` → existing handler.
- Anything else → existing handler ("any user message").

**Stalled-coder detection rule:** after kicking a batch, if no `coder_progress:` comment has landed for a ticket whose most recent `coder_progress:` event is `stage_entered` AND ≥ 30 minutes (default; configurable later via env) have passed without `red_committed` or `coder_needs_help:`, dispatch ONE nudge via `session_notify` to the coder's session id (the `session_id` recorded in the lifecycle log entry for that ticket) carrying the current `develop_session_id` + a one-line "please reply with `coder_progress:` or `coder_needs_help:`". One nudge per stall window; no further action unless the coder replies with `coder_progress:` or `coder_needs_help:`. If still silent after the next stall window, surface `BLOCKED: CODER_NONRESPONSIVE_NO_PROGRESS` with the coder's session id + kickoff timestamp verbatim.
...
- `local_pr_issues_solved < local_pr_issues_found` where the deficit is in inline review-thread comments or issue-conversation comments surfaced by the FINAL `pr-stabilize-watch.sh` run → `BLOCKED: PR_COMMENTS_UNRESOLVED` (the coder missed inline review-thread comments; the fix-now debt is on the coder, not the orchestrator)
- On any BLOCKED state, do **not** present the PR to the user and do **not** say it is ready for human review.
...
> **Idempotency guard:** if the most recent `ticket_report:` (or poller-driven `DEV_LOOP_WAKE`) for the same `<repo>#<n>` has already been surfaced in this session, skip the notify and the wait — the operator is mid-review and the second surface is noise. Extended: skip also when the most recent `coder_progress:` for that `(repo, issue)` has already been surfaced — `coder_progress:` ticks are progress pings, not terminal reports; surfacing each one would be noise.
...
| `session_notify` returns `error: "session_not_found"` (e.g. stale `develop_session_id` after a restart)                                                                                                                 | develop orchestrator loop (inbound wake from coder)                                    | The `ticket_report:` / `feature_report:` issue comment is the **mandatory durable channel**. **Ticket mode:** `scripts/dev-loop-poller.sh` + `scripts/dev-loop-watch.sh` are the ticket-only durable fallback and should wake the develop orchestrator within one poll interval. **Feature mode:** there is **no poller guarantee** for `feature_report:`; wake or resume the develop orchestrator manually so it fetches the durable `feature_report:` from the PRD parent issue before acting. Do not retry inject. (Coder-side counterpart: the coder emits the `session-notify-fallback` block — see `skills/orchestrate/session-notify-fallback.md` — when its outbound notify hits this shape.)                                                                                                                                                                                                                                                                                                                                      |
...
|| `session_notify` admitted to coder but coder never replies (no `ticket_report:` comment within one poll interval) | develop orchestrator | The return channel is broken — most likely the kickoff message had a stale `develop_session_id` or no `return_target` block. Treat the coder as stalled: do NOT pause the batch; dispatch one nudge via `session_notify` to the coder's session id with the orchestrator's current sessionID + the return_target handshake, then continue waiting. If still silent after the next poll, surface `BLOCKED: CODER_NONRESPONSIVE_NO_HANDSHAKE` with the coder's session id verbatim, the kickoff timestamp, and the absence of a `ticket_report:` comment. Do NOT spawn a parallel coder without operator approval — that creates duplicate work on the same branch. |
|| Coder emits `stage_entered` but no `red_committed` / `coder_needs_help:` for ≥ 30 min (§5b.1 stalled-coder detection) | develop orchestrator | The handshake is alive but the coder is making no progress. Dispatch ONE nudge via `session_notify` to the coder's session id carrying the current `develop_session_id` and a one-line "please reply with `coder_progress:` or `coder_needs_help:`". One nudge per stall window. If still silent after the next stall window, surface `BLOCKED: CODER_NONRESPONSIVE_NO_PROGRESS` with the coder's session id + kickoff timestamp verbatim. Do NOT spawn a parallel coder without operator approval. |
|| Coder hits a soft step budget or any non-BLOCKED stall mid-stage and posts a "should I continue or re-kick?" question to the orchestrator | develop orchestrator | The handshake protocol is working. The orchestrator decides: (1) re-kickoff into a fresh session bound to the same worktree (§5a.1) with a handoff pointer, or (2) let the current session push one more bounded pass. If (1), commit the dirty tree as WIP before the kickoff (orchestrator-owned). If (2), reply with the scope of the bounded pass. Do not bother the operator for routine in-flight decisions — only escalate on hard blockers (BLOCKED: codes, provider failure, env unfixable). |
|| Fresh kickoff into a worktree with a live-but-stale prior coder | develop orchestrator | After the fresh session is admitted (HTTP 204, bind_failed false), send a standdown message to the prior coder via `session_notify` in sessionID-mode telling it the orchestrator has re-kicked the work and it can stop. Do NOT delete the prior session — the operator may want to inspect its transcripts. Do NOT pause the batch. |
...
When the feature coder wakes you (via `session_notify`, or after any manual user/operator wake), treat the wake as **advisory only**. There is **no poller guarantee** for `feature_report:` or `feature_progress:`. On a manual wake, first fetch the latest durable `feature_report:` from the PRD parent issue before acting. You do **not** re-verify code-review or CodeRabbit evidence, but you **do** verify the durable `human_review_handoff/v1` contract before surfacing the feature PR.

The feature coder's `feature_progress:` is the visibility stream (same shape as ticket mode `coder_progress:`); `feature_report:` is still the terminal contract. Inbound `feature_progress:` ticks are logged to the lifecycle log and surfaced to the operator only when `next_action_required != none`. Inbound `feature_needs_help:` triggers the §5b.1 envelope handler with the embedded `resume_instruction` verbatim.

- `READY_FOR_HUMAN_REVIEW` → first fetch the raw latest durable `feature_report:` comment from the PRD parent issue, then validate `human_review_handoff/v1` exactly as in §5c (same required fields and count invariants, plus non-empty `docs_paths` and `tickets_state_feature_review`). Missing fields → `BLOCKED: REVIEW_HANDOFF_INCOMPLETE`; mismatched counts or unresolved fix-now debt → `BLOCKED: REVIEW_HANDOFF_INCONSISTENT`; unresolved fix-now debt in `local_pr_*` from inline review-thread comments or issue-conversation comments surfaced by the FINAL `pr-stabilize-watch.sh` run → `BLOCKED: PR_COMMENTS_UNRESOLVED`. Only after the contract passes do you continue.
...
2. **Backup before any write.**
   ...
   ```sql
   CREATE TABLE IF NOT EXISTS project_sandboxes_backup (
     project_id TEXT, sandboxes TEXT, backup_at TEXT
   );
   INSERT INTO project_sandboxes_backup VALUES (
     '<project_id>', '<current sandboxes JSON>',
...
| ----------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `feature_report:` (issue comment on PRD parent, carrying `human_review_handoff/v1`) | feature coder on terminal report                                                                                                                                        | develop orchestrator reads the durable contract first: READY → validate `human_review_handoff/v1` + §8a verify + §8b standard handoff table + §8c-i state:done + §8c-ii merge; `FEATURE_REMEDIATION` → re-batch `remediation:` issues; other BLOCKED → surface + pause. If `session_notify` missed, wake or resume the develop orchestrator manually and fetch this durable report — there is no poller guarantee. |
...
| `coder_progress:` (issue comment on child issue, stage-boundary pings)               | coder session at every stage boundary (ticket-lifecycle §2.6)                                                                                                          | develop orchestrator's `scripts/dev-loop-watch.sh` parses the latest `coder_progress:` per issue into the wake envelope under `coder_progress: { <issue>: <latest event> }`; the poller surfaces `CODER_NEEDS_HELP` and `coder_progress:` deltas in the `DEV_LOOP_WAKE` body. The inbound `session_notify` fast channel also delivers `coder_progress:` directly. The marker is a progress ping, not a terminal report — idempotency guard suppresses re-surfacing of the same event. |
| `coder_needs_help:` (issue comment on child issue)                                  | coder session when stuck (ticket-lifecycle §2.7)                                                                                                                       | develop orchestrator's `§5b.1` envelope handler: dispatch `dev-loop-watch.sh`, then run the embedded `resume_instruction` verbatim (or `session_kickoff` for the "ran out of steps" recovery path). Also surfaced as `CODER_NEEDS_HELP` advisory by `scripts/dev-loop-poller.sh`. |
| `feature_progress:` (comment on PRD parent issue, stage-boundary pings)             | feature coder at every feature-mode boundary (feature-review §F.1)                                                                                                     | develop orchestrator's `§8a` envelope handler: log to lifecycle log; surface only when `next_action_required != none`. No poller guarantee (feature mode is operator/manual wake plus durable fetch). |
| `feature_needs_help:` (comment on PRD parent issue)                                 | feature coder when stuck (feature-review §F.2)                                                                                                                         | develop orchestrator's `§8a` envelope handler: execute the embedded `resume_instruction` verbatim (or re-kick feature coder for the worktree). No poller guarantee. |
| `DEV_LOOP_WAKE: { repo, feature, reason }`                                          | poller (`scripts/dev-loop-poller.sh`) when a ticket delta is detected, or manual operator fallback in ticket mode (see `skills/orchestrate/session-notify-fallback.md`) | develop orchestrator; ignored if no active loop for that feature. The wake is advisory only: re-run `scripts/dev-loop-watch.sh`, then apply §5c. Feature-mode fallback is manual user/operator wake plus durable `feature_report:` fetch in §8a.                                                                                                                                                                   |
...
- **Session messaging is direct.** `session_kickoff` and `session_notify` are plugin tools you call directly. The §5 loop calls `session_kickoff` per ticket, then waits for terminal reports that arrive via `session_notify` (in-session) or the poller.
