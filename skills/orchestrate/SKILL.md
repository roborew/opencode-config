...
When the feature coder wakes you (via `session_notify`, or after any manual user/operator wake), treat the wake as **advisory only**. There is **no poller guarantee** for `feature_report:`. On a manual wake, first fetch the latest durable `feature_report:` from the PRD parent issue before acting. You do **not** re-verify code-review or CodeRabbit evidence, but you **do** verify the durable `human_review_handoff/v1` contract before surfacing the feature PR.
