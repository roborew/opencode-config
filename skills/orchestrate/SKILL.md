...
while true:
  ...
  for entry in batch:
    ...
    # The `directory_match` / `agent_match` fields come from a global-scope
    # `session_list({})` re-list (`agents/session-manager.md` step 5) — the
    # scoped re-list in the previous shape missed worktree-bound sessions
    # and is the source of the historic false-bind bug.
    if not ks.admitted:
