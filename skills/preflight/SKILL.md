...
5. **Compose test file + Docker reachability (verification backend)** — Detect the repo's compose test file and which Docker backend is reachable, so orchestrate can route verify Tasks to the Docker path:
   - `compose_test_file`: `docker-compose.test.yml` | `compose.test.yaml` | `none` — check the impl repo root (`test -f docker-compose.test.yml` then `test -f compose.test.yaml`).
   - `docker`: `ready` | `unavailable` — `command -v docker` succeeds (Docker Desktop / daemon reachable).
   - `sandbox`: `ready` | `unavailable` — from step 4 (`sandbox probe`).
   - If a project has `test_commands` but `compose_test_file: none`, set `verification_gap: true` (surfaced to the readiness gate; not a preflight Block by itself).
6. **Dependencies** — When `package.json` + lockfile exist and `node_modules/` is absent (or README requires install): run **one** documented install using `command_prefix` from the runtime script (`mise exec -- pnpm install`, `asdf exec pnpm install`, `pnpm install`, etc.). Re-check that the package manager resolves.
...
- `compose_test_file`: `docker-compose.test.yml` | `compose.test.yaml` | `none` — the repo's compose test file (verification backend).
- `docker`: `ready` | `unavailable` — whether `docker` is reachable (Docker Desktop / daemon) for the direct-Docker verification path.
- `verification_gap`: `true` | `false` — true when `test_commands` exist but `compose_test_file: none` (surfaced to the readiness gate).
