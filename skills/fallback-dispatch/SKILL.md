...
- `original_skill` — must be a concrete skill file resolvable to `~/.config/opencode/skills/<original_skill>/SKILL.md` (e.g. `developer`, `code-review`, `senior-dev`, `scribe`, `frontend-dev`, `ux-dev`, `worktree-sandbox`, `vision`, `review`, `helper`, `strategist`, `debugger`, `refactor`, `document`, `architecture-auditor`, `codebase-design`). Skill name **must** be supplied explicitly; do not infer from the child report transcript.
- `task_contract` — the verbatim original Task prompt (or a faithful restatement) including all required fields: `impl_repo_path`, `expected_branch`, `is_linked_worktree`, `main_checkout_root`, `branch_policy`, plus role-specific fields (e.g. for `code-review`/`developer`/`frontend-dev`/`ux-dev` Stages: `diff_base`, `files_changed`, `acceptance_to_test`, `red_phase`, `test_commit`, `green_phase`, `implementation_commit`, `test_amendments`, `assertion_delta`, `security_review`; for `scribe`: target path and content; for `review` / `strategist`: artifact / PR context; etc.).
...
- `original_agent` MUST resolve to an **allowed child** (`developer`, `frontend-dev`, `ux-dev`, `test-writer`, `code-review`, `scribe`, `worktree-sandbox`, `vision`, `senior-dev`, `review`, `strategist`, `debugger`, `refactor`, `document`, `architecture-auditor`).
...
| ---------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `developer` / `frontend-dev`                                           | `stage_id` (or `issue_number`), `plan_file` or `repo`, `files_changed`, `tests_run` (with `red_phase`, `test_commit`, `green_phase`, `implementation_commit`, `assertion_delta`, `acceptance_to_test`, `test_amendments`), `acceptance_check_status`, `blockers`, residual_risks, `HANDOFF_COMPLETE`                                                                                      |
| `code-review`                                                          | verdict, confidence, diff review, criterion checklist, RED replay, commands run, sandbox, security review, coverage assessment, remediation, and complete `commit_protocol` (`test_commit_present`, `test_commit_before_implementation`, `test_commit_test_only`, `implementation_commit_present`, `implementation_commit_production_only`, `worktree_clean`, `same_test_red_then_green`) |
...
| `worktree-sandbox`                                                     | mode-specific lifecycle envelope (`env_copy` / `probe_and_create` / `status` / `teardown`)                                                                                                                                                                                                                                                                                                |
