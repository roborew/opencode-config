...
- Bypass verifier / security-reviewer gating on acceptance criteria.
- Bypass checkout identity / `branch_policy`.
- Bypass write restrictions inherited from the original role (`scribe` writes markdown only; `verifier` edits nothing; etc.).
...
- After a successful fallback, the primary resumes the normal workflow at the **next normal gate** (verifier, review gate, etc.). The fallback's completion is graded exactly like the original role's report — same `report_grade: PASS | NEEDS_RETRY | BLOCKED` rubric, fallback envelope is informational.
