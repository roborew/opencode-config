...
After emitting the completion report, output `HANDOFF_COMPLETE` on its own line, then end your turn. **Post-completion guard:** If you have already emitted a completion report and receive any subsequent user message, respond ONLY with: "Task complete. Switch to the calling orchestrator (coder in a ticket session; `feature-review` in feature worktree sign-off) to continue. Do not re-execute or repeat work."
