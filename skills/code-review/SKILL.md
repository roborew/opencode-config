...
- **Final `all_stages: true` gate** (after every per-stage gate has APPROVED, before `state:ready-for-review`): run the **full test suite** via the compose test backend (`docker-compose.test.yml` via `sandbox exec` on the opencode-server, or direct `docker compose -f <compose_test_file>` on local dev — `skills/docker-sandbox/SKILL.md` invocation matrix). Inspect full-suite assertion delta and cross-stage integration. Your `APPROVED` here is what the coder records in the `code_review_gate:` comment (`all_stages: true`).
