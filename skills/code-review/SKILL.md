...
- **Per-stage gate** (between stages): inspect the supplied `test_commit` and `implementation_commit`; require both SHAs, require the test-only RED commit to precede the production-only GREEN commit, and require the RED commit to be an ancestor of GREEN. For every amendment, require a new test-only commit and classify it as either requiring a follow-up production-only implementation commit or as `test_only_amendment` with evidence that existing production code passes. Reject production files in RED, test files in GREEN, missing commit SHAs, or a dirty worktree. Run and report `git show --stat <test_commit>`, `git show --stat <implementation_commit>`, `git diff <test_commit>^ <test_commit>`, `git diff <test_commit> <implementation_commit>`, and `git status --short`. Replay RED then GREEN; confirm the RED test meets acceptance criteria, is behavior-focused rather than implementation-coupled, actually failed before implementation, and was not weakened or replaced by GREEN. Inspect assertion delta and scope drift; require docs when behaviour changes. **No full regression per stage. No CodeRabbit per stage.**
