...
If any `feature:<slug>` issue lacks the `verified` label, **stop** — every issue must carry `verified` (set only on code-review APPROVED) before merge. This is a cheap backstop; architect Mode F accept already refuses unverified issues.
