...
architect / 1    →    develop orchestrator (develop branch)                    architect / 3
                        ...
                        →  coder / feature-review (in feature worktree)
                           full-suite code-review + CodeRabbit + docs
                           + state:done + feature PR + stabilization
                           → one feature_report: comment
                        →  orchestrator final gates + "all reviewed" → feature PR merge
...
- The **feature coder** (`feature-review`) owns the post-batch loop: §0 Bootstrap → full-suite `code-review` → one-shot CodeRabbit (medium/hard) → difficulty gates (medium → `review`; hard → `senior-dev scheduled_review`) → docs (`document` + `scribe`) → `state:done` on every ticket → feature PR via `scripts/feature-finish-pr.sh` → bounded stabilization (max 3 iterations) → tear down the compose backend → one terminal `feature_report:` comment on the PRD parent (mandurable durable channel) + best-effort `session_notify` to `develop_session_id`. Unmet acceptance → `remediation:` GitHub issues via `to-tickets` (`--parent-issue`), returns `BLOCKED: FEATURE_REMEDIATION`; the develop orchestrator re-batches those through the normal ticket pipeline.
- After the feature coder's `READY_FOR_HUMAN_REVIEW`, the develop orchestrator enforces the final gates (per-ticket `code_review_gate:` `all_stages: true` + `verified`; full-suite evidence; CodeRabbit verdict; docs paths; all tickets `state:done`; `gh pr checks` green), prints the merge prompt, and merges the feature PR on "all reviewed" (`gh pr merge --squash --delete-branch=false`). Then emits the spec-handoff string.
...
| Difficulty gate | easy — none; medium — `review` completion summary; hard — `senior-dev scheduled_review` |
| One-shot CodeRabbit | medium/hard only — `PASS` or `BLOCKED` |
