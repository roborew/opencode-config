...
| Build                       | **each work repo**                   | **orchestrate** on `develop` (new session)                               | Paste handoff / `feature:<slug>`         | Wait until the feature PR exists                                                                                                                                                      | Develop orchestrator owns `opencode/feat-<slug>` + per-ticket worktrees; for each ticket it dispatches `worktree-manager create_ticket` then calls the `session_kickoff` plugin tool (the composite kickoff tool — scoped-list → reuse-or-create → inject with `?directory=`) to inject the kickoff pointer into the auto-started GUI session. That auto-started session IS the **coder** primary agent (loading `ticket-lifecycle`), self-bootstraps from the kickoff pointer + GitHub, runs every `stages[]` entry + sub-PR stabilization, and posts one `ticket_report:` comment. |
| Review / verify / docs / PR | **same work repo, feature worktree** | **coder** (new session, kicked by orchestrator) → loads `feature-review` | (no menu — kicks the loop automatically) | Full-suite `code-review`, PR-side CodeRabbit (medium/hard), difficulty gates, docs, `state:ready-for-feature-review` accept, feature PR, bounded stabilization, one `feature_report:` | All PR readiness, ticket acceptance, and feature PR open + stabilization.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
architect / 1    →    develop orchestrator (develop branch)                    architect / 3
                         └─→ create_ticket + session_kickoff (per ticket, auto)
                              (auto-started GUI session IS the coder session;
                               kickoff pointer + GitHub = source of truth)
                        ...
                        └─→ sub-PRs merge into opencode/feat-<slug>
                         →  coder / feature-review (in feature worktree)
                            ...
                            + state:ready-for-feature-review + feature PR + stabilization
...
- The **develop orchestrator** (`orchestrate`) is the _only_ persistent session in the impl repo. It lives in the `develop` branch, owns all `worktree-manager` calls and remote-branch deletes, and never executes tickets itself. For each runnable ticket it dispatches `worktree-manager create_ticket`, then calls the `session_kickoff` plugin tool directly (`plugins/session-manager.js`) — the composite kickoff (scoped-list → reuse-or-create → inject with `?directory=`) replaces the previous `session-manager` subagent layer. No brief file is written; the kickoff pointer is the contract and the coder session reconstructs from the kickoff message + GitHub. There is **no** `task`-tool ticket dispatch — subagents would inherit the develop cwd and `scripts/checkout-contract.sh --verify` would reject them.
- **Coder sessions** are auto-started GUI sessions for each ticket worktree, running the `coder` primary agent loading `ticket-lifecycle`. They own the full inner loop: §0 Bootstrap (read the kickoff pointer via the read tool + reconstruct from GitHub when missing) → silent verification-backend bring-up (`worktree-sandbox` `mode: probe_and_create` — drives `plugins/sandbox.js` to resolve `sandbox_id`, `backend`, `compose_test_file`, build+warm the test backend; `compose_test_file: none` → `BLOCKED: ENV_BLOCKED`) → every `stages[]` entry (test-writer failing test in a test-only RED commit → owner implementation in a separate production-only GREEN commit → per-stage focused code-review of the pair; any test amendment is its own test-only commit) — stage implementers call the **plugin tool `sandbox_run_test`** from `plugins/sandbox.js` directly (they do not write `docker compose` invocations) → **final `all_stages: true` full-suite gate** via the same `sandbox_run_test` → **local CodeRabbit pre-flight (`ticket_coderabbit_preflight` — correctness/obvious-bugs/risky-changes scope; findings applied as fix-now suggestions before the sub-PR)** → sub-PR open → PR stabilization (max 3 iterations) → tear down the compose backend (`worktree-sandbox` `mode: teardown` → plugin `sandbox_destroy`) → one terminal `ticket_report:` comment (mandatory durable channel) + best-effort `session_notify` to `develop_session_id`. Mid-stage escalation to `senior-dev` is unattended (no operator confirmation — the only human gate is PR review); provider fallback (`kilo-fallback` → `openrouter-fallback`) is layered on top for failed children with a complete `fallback_context`.
- **Human review handoff is a contract, not free-form prose.** Both `ticket_report:` and `feature_report:` now carry `human_review_handoff/v1` with a fixed summary/counts block (`what_was_done`, `wrap_up`, test-report count, CodeRabbit found/solved, local PR found/solved, combined fix-now found/resolved, PR / issue / PRD links, notify status). The develop orchestrator treats wake messages as advisory only, fetches the durable report, validates the contract, and only then shows the standard markdown review table to the human. Missing or inconsistent fields block READY (`REVIEW_HANDOFF_INCOMPLETE` / `REVIEW_HANDOFF_INCONSISTENT`).
- The develop orchestrator merges each sub-PR after human approval with a non-squashing merge so the reviewed RED/GREEN commits remain in the feature branch, deletes the ticket worktree + remote ticket branch, and re-batches until `scripts/dev-loop-batch.sh` exits 1. Then it **kicks the feature coder** in the feature worktree (same `coder` agent, loading `feature-review`).
- The **feature coder** (`feature-review`) owns the post-batch loop: §0 Bootstrap → full-suite + PR-side CodeRabbit gate + medium completion summary (all dispatched to `code-review`, `feature_coderabbit_gate` for the gate) → difficulty gates (medium → `code-review` completion summary; hard → `senior-dev scheduled_review`) → docs (`document` `feature_docs` + `scribe`) → `state:ready-for-feature-review` on every ticket → feature PR via `scripts/feature-finish-pr.sh` → bounded stabilization (max 3 iterations) → tear down the compose backend → one terminal `feature_report:` comment on the PRD parent (mandatory durable channel) + best-effort `session_notify` to `develop_session_id`. That `feature_report:` carries the same `human_review_handoff/v1` contract and is not considered READY until the contract validates cleanly. Unmet acceptance → `remediation:` GitHub issues via `to-tickets` (`--parent-issue`), returns `BLOCKED: FEATURE_REMEDIATION`; the develop orchestrator re-batches those through the normal ticket pipeline.
- After the feature coder's `READY_FOR_HUMAN_REVIEW`, the develop orchestrator verifies every child carries `state:ready-for-feature-review` (§8a), then on human "all reviewed" runs §8c-i (mark every ticket `state:done`) → §8c-ii (`gh pr merge --merge --delete-branch=false`, preserving reviewed RED/GREEN commits) → §9 close-loop. It does **not** re-verify code-review or CodeRabbit evidence — the feature coder's terminal report plus the human approval are its only gates. Then it emits the spec-handoff string.

### Per-slice TDD commit contract

Every `stages[]` entry is a reviewable commit sequence:

1. `test-writer` creates the intended failing test and commits it as a **test-only RED commit**.
2. The stage owner implements the behavior and commits it separately as a **production-only GREEN commit**.
3. If the test is corrected or expanded, each change is a separate test-only amendment commit, never folded into RED or GREEN.
4. The focused `code-review` validates the RED/GREEN pair and all listed amendment commits against the stage scope and acceptance criteria. A green test run without the pair is not sufficient for stage approval.

See [rules/testing.md](../rules/testing.md#per-slice-tdd-commit-contract) for the shared testing rule.

### Hard reminders

- Sign-off of the **work** (full-suite verification, `state:ready-for-feature-review`, docs, PR merge-ready) = **feature coder** in the feature worktree, kicked by `orchestrate`.
- Final accept (`state:done` on every ticket) and the **feature PR merge** = **orchestrator** on "all reviewed" (after the feature coder's `feature_report:`).
...
| `state:ready-for-feature-review` on every ticket | After feature coder opens the feature PR; issues stay **open** until spec merge                             |
