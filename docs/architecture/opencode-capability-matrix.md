...
| CodeRabbit gate (completion) | `orchestrate` → `review` | `code-review` (+ `review` on `load: full`) | Parent: `load: full` on CodeRabbit Task | `CODERABBIT_GATE` + findings; optional issue comment | Before difficulty gates / `ready-for-review` |
