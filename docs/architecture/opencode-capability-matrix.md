...
| Worktree env symlinks (opt-in) | `worktree-env` | `worktree-env` | Parent: `load: full` once per bootstrap; trust canonical evidence | Symlink report with `wt_root`, `main_root`, per-file `readlink` | `preflight` (verify only) |
