...
| CodeRabbit gate (completion) | `orchestrate` → `review` | `code-review` (+ `review` on `load: full`) | Parent: `load: full` on CodeRabbit Task; **once** per artifact/feature | `CODERABBIT_GATE` + findings; feature completion summary | After last verifier / queue exhausted; before difficulty gates & architect — **not** per issue |
