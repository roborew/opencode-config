...
| Docker compose build/test + optional review URL (Sysbox sibling) | `developer`, `frontend-dev`, `verifier` (probe: `preflight`) | `docker-sandbox` | Load when Compose/Docker or web review needed and `sandbox` on PATH / preflight `sandbox: ready` | `sandbox exec` logs; optional `https://{slug}.{apex}` | `OPENCODE_SANDBOX_ENABLED` (expose: localhost publish + host tunnel public hostname + `OPENCODE_SANDBOX_REVIEW_DNS`) |
