...
| Coordinator             | `orchestrate`                                | smart      | Execute stages, grade children, helper recovery, optional `review` (medium) / `senior-dev`+`helper` (hard) after final verifier, dispatch scribe. Plan picker lists **active** `.plan/*.md` only (excludes `*.completed.md`). Mandatory startup: **`worktree-env`** (linked worktree `.env` / `.env.local`) then **`developer`** + `preflight`.                       |
...
Environment readiness (`worktree-env` + preflight) is mandatory before implementation work; do not require artifact writes for preflight output. Claude Context readiness runs after the env gate on fresh sessions.
