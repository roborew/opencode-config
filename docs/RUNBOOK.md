...
- **Commit each slice explicitly** using the stage `commit_message` convention from `opencode-task-yaml` (append `Refs: #<issue_number>`): first a test-only failing **RED commit**, then a separate production-only **GREEN commit**. Each test amendment is another separate test-only commit. The per-stage `code-review` validates the RED/GREEN pair and any amendments before the stage is accepted. Local commits are fine; remote pushes are not part of the per-issue loop.
...
- **Preserve reviewed commit history:** approved ticket and feature PRs use a non-squashing merge so the test-only RED and production-only GREEN commits remain auditable after integration.
...
- **Built-in agents:** `plan` uses DeepSeek V4.1 Flash; `build` uses DeepSeek V4.1 Flash for generic/quick tasks.
...
- **Coder** (`coder`) — primary agent hosted inside each worktree (auto-started GUI session for tickets, kicked by orchestrator for feature worktrees). Loads `ticket-lifecycle` in a ticket worktree or `feature-review` in the feature worktree. In ticket mode: owns every stage (test-writer test-only RED commit → owner production-only GREEN commit → per-stage code-review of the pair; test amendments are separate commits), runs the **final `all_stages: true` full-suite gate** via the compose test backend (`docker-compose.test.yml` via `sandbox exec` on opencode-server, or direct `docker compose -f <file>` on local dev; see `rules/verification.md` and `skills/docker-sandbox/SKILL.md`), then dispatches `code-review` for the **local CodeRabbit pre-flight** (`ticket_coderabbit_preflight` — correctness/obvious-bugs/risky-changes scope, findings applied as fix-now suggestions) before the sub-PR opens, escalates to senior-dev unattended (no operator confirmation — the only human gate is PR review), falls back failed children to kilo/openrouter via `fallback-dispatch`. In feature mode: dispatches `code-review` for the full suite + **PR-side CodeRabbit gate** (`feature_coderabbit_gate`, medium/hard) + medium completion summary, inside the feature verification loop; owns docs, `state:ready-for-feature-review`, the feature PR, bounded stabilization, and the terminal `feature_report:`. The develop orchestrator sets final `state:done` after human review. Holds `session_notify` to inject the terminal `ticket_report:` (ticket mode) or `feature_report:` (feature mode) back into the develop orchestrator session. Never writes files directly — delegates everything.
...
| Ticket executor wrapper | `coder`                                              | smart      | Per-ticket inner loop: §0.0 Handshake (push `opencode/feat-<slug>` + own ticket branch from the worktree cwd via `developer`) → §0 Bootstrap (kickoff pointer + GitHub) → silent preflight (compose test backend) → every `stages[]` entry → final-gate full suite via `docker-compose.test.yml` → local CodeRabbit pre-flight → sub-PR + stabilization → terminal `ticket_report:` + `session_notify`. Senior-dev escalation unattended; provider fallback layered on top. Non-writing (`edit: deny`). Feature mode: §0.0 Handshake (push feature branch from the worktree cwd) → full verification loop incl. PR-side CodeRabbit (medium/hard), difficulty gates, docs, `state:ready-for-feature-review`, feature PR, bounded stabilization, terminal `feature_report:`. The develop orchestrator sets final `state:done` after human review. |
...
## Model routing policy (Zen, cost-tiered)

All agents run on the Zen providers (`opencode` / `opencode-gpt`) except `security-reviewer` and full-audit escalation, which use `opencode-go/qwen3.8-max` — the only Go-endpoint model retained. Kilo and OpenRouter remain as cross-provider fallbacks. Route by task shape, not agent prestige. Keep one deliberate fallback per important role.
...
5. `orchestrate` runs §0 Bootstrap: dispatch ONE `developer` Task (`load: minimal`) running `scripts/checkout-contract.sh`; require `status: ok` and capture `is_linked_worktree` + `branch_policy`. The orchestrator itself has no bash — every shell invocation is a delegated Task. `CHECKOUT_CONTRACT_FAILED` is surfaced verbatim and stops the session (no improvised alternative menu). If `is_linked_worktree: true`, dispatch ONE `worktree-sandbox` Task (`mode: env_copy`) to copy `.env` / `.env.local` from the main checkout into the worktree (the agent drives the plugin tool `env_copy` from `plugins/sandbox.js`; it never writes bash). Preflight belongs to coder sessions (`ticket-lifecycle` §0.3 dispatches `worktree-sandbox` `mode: probe_and_create` silently); the orchestrator never prompts for preflight.
...
9. Preflight is coder-owned. The orchestrator never prompts for preflight and never dispatches `worktree-sandbox` (mode: probe_and_create). Per-ticket preflight is rerun only when the ticket surfaces `ENV_BLOCKED` remediation in its coder session.
10. `orchestrate` creates the feature worktree via a `worktree-manager` Task (`create_feature`) and captures the returned `branch` field (e.g. `opencode/feat-<slug>`) — it does **not** push the feature branch itself. The push is owned by the **coder session's §0.0 Handshake** (`skills/ticket-lifecycle/SKILL.md` §0.0 for ticket worktrees, `skills/feature-review/SKILL.md` §0.0 for the feature worktree): the coder dispatches a `developer` Task with `gh api create_ref` to create the remote feature branch if missing, then `git push -u origin <feature_branch>` and (ticket mode only) `git push -u origin <expected_branch>` from the worktree cwd. The orchestrator then dispatches `worktree-manager` `create_ticket` with `feature_branch` set to the captured branch (the safety link that prevents tickets from forking off `develop`/`main`) and calls the `session_kickoff` plugin tool directly (`plugins/session-manager.js`) with `agent: "coder"` and a short pointer message. The plugin writes nothing; the kickoff pointer is the contract and the coder session reconstructs from it + GitHub. Ticket work is **never** dispatched via the `task` tool — subagents inherit the `develop` cwd and `scripts/checkout-contract.sh --verify` would reject them.
11. Each coder session owns the full inner loop for one ticket: `ticket-lifecycle` §0 Bootstrap (kickoff pointer + GitHub reconstruction) → silent preflight (compose test backend) → every `stages[]` entry as a test-only failing RED commit followed by a separate production-only GREEN commit (with any test amendments in their own commits) → per-stage `code-review` validating that commit pair → final-gate full suite (`all_stages: true` via compose) → **local CodeRabbit pre-flight** (`ticket_coderabbit_preflight` — findings applied as fix-now suggestions before the sub-PR) → sub-PR + stabilization → terminal `ticket_report:` + `session_notify`. Stages + per-stage focused `code-review` are dispatched inside the coder session, not by the orchestrator.
12. The coder session holds `session_notify`; it injects the terminal `ticket_report:` pointer back into the develop orchestrator session. The develop orchestrator wakes via: (i) in-session `session_notify` (primary), (ii) ticket-only poller `scripts/dev-loop-poller.sh` firing `DEV_LOOP_WAKE` for durable ticket deltas, (iii) any user message — dispatch a `developer` Task (`load: minimal`) to run `scripts/dev-loop-watch.sh` first. Feature-mode fallback is manual wake/resume plus durable `feature_report:` fetch by the orchestrator; there is no poller guarantee for `feature_report:`.
13. PR-approval gate: per `READY_FOR_HUMAN_REVIEW`, the orchestrator notifies the user ("PR ready for review: <pr_url>") and waits for "yes, happy with that ticket" — the single human gate per PR. The orchestrator then dispatches a `developer` Task with explicit `cd`/`git -C` to `gh pr merge --merge --delete-branch=false` and fast-forwards `opencode/feat-<slug>`. It dispatches `worktree-manager` `delete` for the ticket worktree and a `developer` Task for `git push origin --delete opencode/ticket-<issue>-<slug>-<abbrev>` (the only branch-deleting actor).
...
18. **Spec `feature-complete`** (post-merge): verify-only ceremony — confirm every `feature:<slug>` issue is `state:done` + `verified` and the impl feature PRs are **MERGED** (merged by the develop orchestrator on "all reviewed"), then close child issues + PRD parent + delivery record. By this point the develop orchestrator already handled the final human review + merge; spec `feature-complete` only verifies/closes.
...
**Ubuntu Sysbox sandboxes (optional):** When the utilities opencode-server stack enables Sysbox siblings (`OPENCODE_SANDBOX_ENABLED=1`), the `sandbox` CLI is on PATH inside the server container. `plugins/sandbox.js` probes softly (`sandbox: ready|unavailable`); `unavailable` falls back to direct `docker compose` automatically (verification backend is not downgraded — the two backends are interchangeable). **Orchestrate does not load skill `docker-sandbox`** — it detects compose/Docker need and instructs `developer` / `frontend-dev` / `code-review` Tasks to load it; stage implementers call `sandbox_run_test` from the plugin directly. The `worktree-sandbox` subagent drives the entry/exit choreography (probe_and_create / teardown) via the same plugin — agents never write `sandbox create|exec|destroy` or `docker compose` invocations themselves. App Infisical for Compose comes from repo `.env` (`./scripts/setup.sh projects …`, then `worktree-sandbox` `mode: env_copy` on linked worktrees) — not OpenCode server Infisical injection. Do not invent host Docker/Sysbox usage when the probe fails. **Not** Cloudflare Workers Sandbox (`skills/cloudflare/references/sandbox/`).
...
| High-volume execution        | `developer`, `frontend-dev`, `coder`, `orchestrate`, `helper`, `debugger`, `refactor`, `review` | `opencode/glm-5.3-flash` (developer/helper/refactor/review); `opencode/deepseek-v4-pro` (debugger); `kilo/minimax/minimax-m3` (frontend-dev, coder, orchestrate) | `kilo/minimax/minimax-m3` when Zen quota is exhausted; same model retained on Kilo when coder/orchestrate runs on Kilo                                  |
| Fast utility / setup         | `worktree-sandbox`, `document`, `doc-reviewer`, `stack-bootstrap`                               | `opencode/glm-5.3-flash`                                                                                                                                         | `opencode/glm-5.3` if GLM 5.3 Flash fails                                                                                                               |
| Docs/PRD writer              | `scribe`                                                                                        | `opencode/glm-5.3-flash`                                                                                                                                         | `opencode/glm-5.3` if GLM 5.3 Flash fails                                                                                                               |
| Independent code-review gate | `code-review`                                                                                   | `kilo/minimax/minimax-m3`                                                                                                                                        | `opencode-gpt/gpt-5.6-luna` for escalation/high-risk only                                                                                               |
| Architecture assessment      | `architecture-auditor`                                                                          | `opencode/glm-5.3` (feature-impact)                                                                                                                              | `opencode-go/qwen3.8-max` for full audits                                                                                                               |
| Feature decomposition        | `strategist`                                                                                    | `opencode/deepseek-v4-pro`                                                                                                                                       | `opencode-gpt/gpt-5.6-luna` for hard cross-repo                                                                                                         |
| Senior escalation / review   | `senior-dev`                                                                                    | `opencode/kimi-k3`                                                                                                                                               | `opencode-go/qwen3.8-max` when Kimi K3 is unavailable                                                                                                   |
| Security analysis            | `security-reviewer`                                                                             | `opencode-go/qwen3.8-max`                                                                                                                                        | `opencode/claude-opus-4-8` only for user-approved high-severity escalation                                                                              |
| Vision / visual              | `vision`, `designer`, `ux-dev`                                                                  | `opencode/gemini-3-flash` (retained)                                                                                                                             | Current configured                                                                                                                                      |
| Teaching                     | `mentor`                                                                                        | `opencode/qwen3.7-max`                                                                                                                                           | —                                                                                                                                                       |
| Generic / built-in           | `plan`, `build`                                                                                 | `opencode/qwen3.7-plus` (plan); `opencode/glm-5.3-flash` (build)                                                                                                 | —                                                                                                                                                       |
...
- `code-review` (per-ticket and per-stage): verifies against the issue's `opencode-task-yaml` acceptance criteria + per-stage scope + the committed test-only RED / production-only GREEN pair, including any separate test-amendment commits. Returns `APPROVED` / `NEEDS_CHANGES` / `BLOCKED`.
...
### Per-slice commit contract

The minimum review unit for a TDD slice is a committed pair, not merely a green test run:

- The failing test is committed first as a **test-only RED commit**.
- The implementation follows as a separate **production-only GREEN commit**.
- Any test correction or expansion is a separate test-only amendment commit; it must be listed and revalidated rather than folded into RED or GREEN.
- The focused `code-review` validates the RED/GREEN pair, the amendment commits, the stage scope, and the acceptance evidence before returning `APPROVED`.

This contract applies inside each ticket coder session; the feature coder's full-suite review consumes the already-reviewed slice history.
...
The poller is idempotent — state files in `~/.local/state/opencode/dev-loop/<owner-repo>__<feature-slug>.json` dedupe wakes per repo+feature snapshot, using meaningful ticket deltas (`ticket_report`, `pr_state`, `out_of_band_merged`, `verified_drift`). A `DEV_LOOP_WAKE` for a feature with no active loop in the develop orchestrator's lifecycle log is silently ignored ("ignore if not yours").
...
- Every `stages[]` slice has a test-only failing RED commit, a separate production-only GREEN commit, and separate test-amendment commits when needed; per-stage `code-review` validates the complete pair.
