...
- **`context7`**: Up-to-date docs for 9000+ external libraries, routed through MCPJungle. The OpenCode configuration uses `MCPJUNGLE_OPENCODE_TOKEN` from the server runtime environment; upstream MCP credentials remain owned by the MCPJungle Infisical project. Use when framework/library API behavior is uncertain. Limit to 3 calls per question.
