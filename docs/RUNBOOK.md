...
**Ubuntu Sysbox sandboxes (optional):** When the utilities opencode-server stack enables Sysbox siblings (`OPENCODE_SANDBOX_ENABLED=1`), the `sandbox` CLI is on PATH inside the server container. Preflight probes softly (`sandbox: ready|unavailable`); `unavailable` is not a hard fail (typical Mac / `OPENCODE_SANDBOX_MODE=off`). Stage `test_commands` may wrap `sandbox exec --id <slug> -- …` for compose build/test — see skill `docker-sandbox`. Do not invent host Docker/Sysbox usage when the probe fails.
