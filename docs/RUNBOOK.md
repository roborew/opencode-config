...
- **`mcpjungle`**: The authenticated gateway for all managed upstream MCP servers. The OpenCode configuration uses `MCPJUNGLE_OPENCODE_TOKEN` from the server runtime environment; each upstream's credentials remain owned by the MCPJungle Infisical project. Context7 is currently available through this gateway for up-to-date library documentation; use it when framework/library API behavior is uncertain and limit to 3 calls per question.
...
**Review-app DNS:** Feature URLs use skill `docker-sandbox` (`sandbox expose` for localhost publish of nested Caddy) plus `cloudflare-api` for tunnel public hostname → `https://127.0.0.1:<hostPort>` with **No TLS Verify ON** (never copy expose’s `http://` scheme; never HTTP service type) and optional CNAME upsert to the **existing** host tunnel — never `cloudflared tunnel create`. Re-auth with utilities `./scripts/setup.sh mcp-auth cloudflare-api` (Zone DNS Edit + Tunnel Edit on the existing tunnel).
