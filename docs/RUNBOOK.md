...
- **Outer-loop coordinator** (`orchestrate`) runs from `develop`. Reads `## Difficulty` from the artifact (`easy` \| `medium` \| `hard`; default `medium` if missing). Owns `opencode/feat-<slug>`, batch kickoff of one **coder** session per ticket (via `worktree-manager` + `session_notify`), PR approval gate (single human gate), merge + worktree + remote-branch cleanup, `HANDOFF_TO_FEATURE_ARCHITECT` handoff when all tickets land. **Never** executes tickets — coder sessions do. CodeRabbit is one-shot at feature sign-off in `architect-feature-signoff`. **Skills:** `orchestrate` (bootstrap + work selection + feature worktree + batch kickoff + wake contract + PR-approval gate + merge/cleanup + handoff).
...
- **Execution subagents** (`developer`, `frontend-dev`, `ux-dev`, `test-writer`) — coding agents dispatched by the **coder** primary agent. Architect and orchestrate never invoke them directly. `frontend-dev` uses MiniMax M3 for JSX/CSS/visual output. `ux-dev` generates HTML-only framework-agnostic prototypes from design briefs into `.prototype/<slug>/`.
- **Coder** (`coder`) — primary agent hosted inside each ticket worktree (auto-started GUI session). Loads `ticket-lifecycle`, owns every stage (test-writer RED → owner GREEN → per-stage code-review), runs the **final `all_stages: true` full-suite gate** via the compose test backend (`docker-compose.test.yml` via `sandbox exec` on opencode-server, or direct `docker compose -f <file>` on local dev; see `rules/verification.md` and `skills/docker-sandbox/SKILL.md`), escalates to senior-dev unattended (no operator confirmation — the only human gate is PR review), falls back failed children to kilo/openrouter via `fallback-dispatch`. Holds `session_notify` to inject the terminal `ticket_report:` back into the develop orchestrator session. Never writes files directly — delegates everything.
- **Senior-dev** (`senior-dev`) — subagent with two explicit modes: `escalation_fix` (mid-stage unblocker; the **coder** dispatches this unattended when a stage exhausts its retry budget or is marked hard/senior — no user confirmation) and `scheduled_review` (hard-difficulty read-only gate inside `architect-feature-signoff`). `escalation_fix` returns `HANDOFF_TO_DEVELOPER` to the wrapping coder, which resumes with `developer` for remaining stage work.
...
| Coordinator             | `orchestrate`                                | smart      | Outer loop on `develop`: feature worktree, batch kickoff of coder sessions, PR approval gate, merge + cleanup, `HANDOFF_TO_FEATURE_ARCHITECT`. Plan picker lists **active** `.plan/*.md` only (excludes `*.completed.md`). Startup: optional preflight prompt; **checkout identity gate always**; **`worktree-env`** + **`preflight`** only when user opts in. |
| Ticket executor wrapper | `coder`                                      | smart      | Per-ticket inner loop: §0 Bootstrap (brief file + GitHub) → silent preflight (compose test backend) → every `stages[]` entry → final-gate full suite via `docker-compose.test.yml` → sub-PR + stabilization → terminal `ticket_report:` + `session_notify`. Senior-dev escalation unattended; provider fallback layered on top. Non-writing (`edit: deny`). |
...
| Operator escalation     | `senior-dev`                                 | smart      | Escalation: unattended when dispatched from coder (mid-stage retry-budget exhaustion or hard/senior stage); operator + user confirm only when dispatched from architect/orchestrate. **Hard** completion gate: auto-invoked post-code-review for scheduled review.                                                                                    |
...
- **Skill:** Each agent may load only its core skill(s). No `skill: { "*": "allow" }`. Explicit allow per skill (e.g. `architect-plan` + `architect-review` for architect; `orchestrate` for orchestrate; `ticket-lifecycle` for coder; `developer` for developer; `preflight` for **`preflight`**; `worktree-env` for **`worktree-env`**).
...
7. `orchestrate` presents the **work-selection menu** verbatim. On `develop`/`main`/`master` (Menu A): **(1)** Start a new feature — `feature:<slug>` then run every ticket end-to-end to a ready-for-review PR; **(2)** Resume a feature — reattach to an existing `feature:<slug>` or ticket worktree; **(3)** Remediation loop — re-check PR feedback / CI (routes to architect option 4 → R); **(4)** Something else. Inside a worktree (Menu B): **(1)** Build / refresh in Sysbox sandbox (compose build/test + optional review URL); **(2)** Remediation loop; **(3)** Something else. (A ticket worktree belongs to its coder session — switch the session's agent to `coder` and say `begin`; the orchestrator here owns sandbox + remediation routing only.)
...
11. Each coder session owns the full inner loop for one ticket: `ticket-lifecycle` §0 Bootstrap (brief file + GitHub reconstruction) → silent preflight (compose test backend) → every `stages[]` entry → final-gate full suite (`all_stages: true` via compose) → sub-PR + stabilization → terminal `ticket_report:` + `session_notify`. Stages + per-stage focused `code-review` are dispatched inside the coder session, not by the orchestrator.
...
14. **CodeRabbit gate** (one-shot at feature sign-off, not per stage/issue/remediation): runs inside `architect-feature-signoff` after the last ticket merges into `opencode/feat-<slug>` — medium/hard invokes CodeRabbit on all changed files against `develop`; easy skips. Never per stage, never per ticket, never after a remediation push.
15. **Difficulty completion gates** run in the feature-architect session (`architect-feature-signoff`), not in the develop orchestrator: **easy** — none; **medium** — `review` with the artifact + completion summary (+ CodeRabbit findings); **hard** — `senior-dev` `scheduled_review`, then `helper` (strategy conformance).
16. **Post-PR stabilization:** `scripts/feature-finish-pr.sh` runs in the feature-architect session, then `pr_stabilization` collects PR checks/comments and user acceptance feedback, classifies findings, executes fix-now items through developer→code-review, and presents checkpoints until the user finalizes. The develop orchestrator re-batches remaining tickets between sub-PRs; the feature-architect session owns stabilization for the final feature PR.
...
**Senior-dev escalation (coder: unattended; orchestrate/architect: operator-confirmed):** When a **coder** stage exhausts its retry budget (2 `NEEDS_CHANGES` from `code-review`) or the stage is marked hard/senior, the coder dispatches `senior-dev` `escalation_fix` **unattended** (no user confirmation — the only human gate is PR review). When **orchestrate** or **architect** dispatches `senior-dev` mid-stage, the operator must confirm before the escalation. **Exception:** for **`Difficulty: hard`**, after all stages pass the final code-review, the feature-architect session invokes `senior-dev` for **scheduled post-implementation review** without that confirmation (not the same as mid-stage escalation).
...
- Senior-dev: from **coder**, unattended mid-stage **escalation** (no user confirmation; only human gate is PR review); from **architect/feature-architect**, user confirmation for mid-stage **escalation**; **no** confirmation for **hard** Difficulty scheduled post-code-review review.
