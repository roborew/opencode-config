...
14. **CodeRabbit gate** (after final verifier, before difficulty gates): **medium/hard** — orchestrate Tasks **`review`** with `execution_mode: orchestrate_coderabbit_gate` and **`code-review`** skill; BLOCKED → developer/frontend-dev fixes → verifier → re-gate (max 2). **easy** — skip.
