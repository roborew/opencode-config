...
16. When gates complete: orchestrate prints the mandatory **`### CodeRabbit`** completion block (ran, CLI command, review runs, remediation fixes applied, final gate, finding counts) — then **"Implementation complete. Switch to architect for review and documentation sign-off."** Architect still runs Mode B review + docs (authoritative sign-off).
