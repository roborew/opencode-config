...
**Stage**:
One TDD slice within an issue's `stages[]` array; the **coder** session (per-ticket inner loop) dispatches `test-writer` for a failing test-only RED commit → `developer`/`frontend-dev`/`ux-dev` for a separate production-only GREEN commit → `code-review` to validate the committed pair. Any test correction or expansion is a separate test-only amendment commit. The **feature coder** session (per-feature worktree, loading `feature-review`) dispatches the same implementer Tasks for the full-suite `code-review` gate.
_Avoid_: Step, phase file

**Per-slice commit contract**:
The minimum TDD evidence for a stage is a test-only failing RED commit followed by a separate production-only GREEN commit; test amendments are separate commits, and `code-review` validates the pair plus those amendments.
_Avoid_: A single mixed test-and-production commit, treating a green run as sufficient evidence
...
- **Orchestrate** runs the **outer loop** (feature worktree + batch kickoff of coder sessions + PR approval + merge/cleanup); **coder** sessions run the inner loop for each ticket (each stage's RED/GREEN commit pair and focused review → final-gate full-suite → local CodeRabbit pre-flight → sub-PR + stabilization → `ticket_report:`). When every ticket merges into `opencode/feat-<slug>`, the orchestrator kicks the **feature coder** in the feature worktree (same `coder` agent, loading `feature-review`) — it owns the final verification (full-suite `code-review`, PR-side CodeRabbit on medium/hard, difficulty gates, docs, `state:ready-for-feature-review` on every ticket, feature PR, bounded stabilization, `feature_report:`). The develop orchestrator sets `state:done` after human `all reviewed` and merges the feature PR — it never re-verifies code-review or CodeRabbit evidence; coder terminal reports plus human approval are its only gates.
- **Architect** is design & constraints only: planning (spec → PRD → fanout), impl-repo planning front door (`to-tickets`, `audit`, `setup-skills`, `explore`, `debug`/`refactor` planning), and post-PR feedback routing to the impl `orchestrate` session. **All sign-off duty removed** — the feature coder owns the pre-merge `feature-review` loop; the develop orchestrator owns final `state:done` + feature PR merge after human review.
