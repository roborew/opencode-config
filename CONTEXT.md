...
- **Orchestrate** runs the **outer loop** (feature worktree + batch kickoff of coder sessions + PR approval + merge/cleanup); **coder** sessions run the inner loop for each ticket (Stages → final-gate full-suite → local CodeRabbit pre-flight → sub-PR + stabilization → `ticket_report:`). When every ticket merges into `opencode/feat-<slug>`, the orchestrator kicks the **feature coder** in the feature worktree (same `coder` agent, loading `feature-review`) — it owns the final verification (full-suite `code-review`, PR-side CodeRabbit on medium/hard, difficulty gates, docs, `state:done`, feature PR, bounded stabilization, `feature_report:`). The orchestrator merges the feature PR on "all reviewed" — it never re-verifies code-review or CodeRabbit evidence; coder terminal reports plus human approval are its only gates.
