...
**CodeRabbit** (`skills/code-review`, from [coderabbitai/skills](https://github.com/coderabbitai/skills)): two runs, two roles — both dispatched by **coder** sessions via `review`, never by the orchestrator. (a) **Local pre-flight per ticket** (`ticket_coderabbit_preflight`): after the final-gate full suite, before the sub-PR opens; scoped to correctness, obvious bugs, and risky changes; findings are fix-now suggestions applied in-worktree. (b) **PR-side gate per feature** (`orchestrate_coderabbit_gate`): inside `feature-review` after the full `feature:<slug>` queue merges (`medium`/`hard` only) — style, regressions, cross-branch context, and policy; `PASS` is required before the feature PR is ready for review. Requires CodeRabbit CLI + auth.
