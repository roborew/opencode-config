...
```mermaid
flowchart TB
  A["1. SPEC repo — architect\nPRD: grill-me → to-prd → fanout-issues → issue-expand"]
  B["2. WORK repo — orchestrate\ncreates the feature worktree, kicks a coder session per backlog ticket"]

  subgraph coderLoop ["3. coder (loop, one session per ticket) — skill: ticket-lifecycle"]
    direction TB
    ...
    C4{"NEEDS_CHANGES?"}
    C5["subagent: senior-dev\none-shot escalation fix"]
    C6{"More stages\nin this ticket?"}
    C7["CodeRabbit pre-flight → open sub-PR"]
    C1 --> C2 --> C3 --> C4
    C4 -->|yes| C5 --> C2
    C4 -->|no| C6
    C6 -->|yes| C1
  ...
  D["4. orchestrate — human signoff\nreview + merge the ticket sub-PR"]
  E["5. coder (feature-review) — final signoff\nfull-suite + CodeRabbit gate → feature PR → human 'all reviewed' → merge"]
  F["6. SPEC repo — architect\nfeature-complete: verify state:done + MERGED, close tickets + PRD"]

  A --> B --> coderLoop --> D
  D -->|more tickets in the backlog| coderLoop
  D -->|backlog empty| E --> F

  classDef default fill:#eef1f5,stroke:#5b6470,color:#1a1a1a,stroke-width:1px;
  classDef decision fill:#eef1f5,stroke:#5b6470,color:#1a1a1a,stroke-width:1px;
  class A,B,D,E,F,C1,C2,C3,C5,C7 default
  class C4,C6 decision
```

```text
1. SPEC (start)      2. WORK repo             3. coder (loop, per ticket)                          4. orchestrate       5. coder            6. SPEC (finish)
architect             orchestrate              skill: ticket-lifecycle                              signoff              feature-review      architect
grill-me -> to-prd -> creates worktree +       test-writer (RED) -> developer/frontend-dev/         human reviews +      full-suite +        feature-complete
-> fanout-issues ->   kicks one coder          ux-dev (GREEN) -> code-review -> [NEEDS_CHANGES ->    merges sub-PR ->     CodeRabbit gate ->  verify + close
issue-expand          session per ticket       senior-dev, retry] -> repeat per stage -> CodeRabbit  next ticket (loop)   feature PR -> merge
                                                pre-flight -> open sub-PR
```

### Per-slice TDD commit contract

Within each ticket's `stages[]` loop, the failing test is committed first as a **test-only RED commit**. The owner then commits the implementation separately as a **production-only GREEN commit**. Any test correction or expansion is a separate test-only amendment commit. The per-stage `code-review` validates the RED/GREEN pair and all listed amendments; a green test run alone is not enough to approve the slice.

See [rules/testing.md](rules/testing.md#per-slice-tdd-commit-contract) for the shared rule.
...
| Review / docs / PR | **same work repo, feature worktree** | **coder** (kicked by orchestrator) → loads `feature-review` | (no menu — kicks the loop automatically)         | Verify + sign off + docs + feature PR                  | All PR readiness and ticket acceptance to `state:ready-for-feature-review`; orchestrator marks `state:done` and merges after `all reviewed`; feature review consumes the already-reviewed RED/GREEN slice pairs                                                                                    |
...
During **setup-project**, architect scans implementation repos for duplicate agent docs. Confirmed items are **archived** (not deleted) under `docs/_archive/legacy/`. Open `feature:<slug>` issues should use **issue-expand** before any legacy doc cleanup.
...
| Per-slice TDD commit contract                                 | [rules/testing.md](rules/testing.md#per-slice-tdd-commit-contract)                                                                                           |
...
**CodeRabbit** (`skills/code-review`, from [coderabbitai/skills](https://github.com/coderabbitai/skills)): two runs, two roles — both dispatched by **coder** sessions via **`code-review`**, never by the orchestrator. (a) **Local pre-flight per ticket** (`ticket_coderabbit_preflight`): after the final-gate full suite, before the sub-PR opens; scoped to correctness, obvious bugs, and risky changes; findings are fix-now suggestions applied in-worktree. (b) **PR-side gate per feature** (`feature_coderabbit_gate`): inside `feature-review` after the full `feature:<slug>` queue merges (`medium`/`hard` only) — style, regressions, cross-branch context, and policy; `PASS` is required before the feature PR is ready for review. Requires CodeRabbit CLI + auth.
